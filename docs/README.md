...
- **Real-artemis smoke is opt-in via `pnpm test:smoke`.** `tests/e2e/smoke-real-artemis.test.ts` is gated on `UNIVERSE_E2E_REAL=1` (set only by the script) and points the real `proxy-client` at `UNIVERSE_REAL_PROXY_URL` (default `https://uploads.freecode.camp`). Required env: `UNIVERSE_REAL_TOKEN` (a GitHub token authorized for the test site) and `UNIVERSE_REAL_SITE` (a pre-registered throwaway slug — the smoke does NOT register/delete sites). The 4 tests cover `whoami`, `static ls`, `static deploy` (preview), and `static deploy --promote` — the last one fetches the public URL with cache busting and asserts the response body contains a freshly-deployed marker, which is the closed-loop test for the "sites not updating" complaint. Run example: `UNIVERSE_REAL_TOKEN=ghp_xxx UNIVERSE_REAL_SITE=staff-smoke pnpm test:smoke`.
