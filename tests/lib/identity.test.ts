...
describe("resolveIdentity — priority chain (ADR-016 Q10)", () => {
  ...
  describe("GHA OIDC slot removed (F7)", () => {
    ...
    // PATs / OAuth / App user-to-server tokens — not OIDC ID tokens.
    // CI users supply `$GITHUB_TOKEN` explicitly. Removed slot must
    // NOT short-circuit when ACTIONS_ID_TOKEN_REQUEST_URL/_TOKEN are
    // present.
    it("ignores ACTIONS_ID_TOKEN_REQUEST_* env, falls through to next slot", async () => {
      const r = await resolveIdentity({
        env: mkEnv({
          ACTIONS_ID_TOKEN_REQUEST_URL: "https://gha.example/token",
          ACTIONS_ID_TOKEN_REQUEST_TOKEN: "gha_req",
        }),
        execGhAuthToken: async () => "ghcli_lose",
        loadStoredToken: async () => "device_wins",
      });
      expect(r?.source).toBe("device_flow");
      expect(r?.token).toBe("device_wins");
    });
  });

  describe("slot 3 — device-flow stored token (`universe login`)", () => {
    it("uses stored token when no env match", async () => {
      const r = await resolveIdentity({
        env: mkEnv({}),
        execGhAuthToken: async () => "should_not_run",
    ...
    it("trims whitespace from stored token", async () => {
      const r = await resolveIdentity({
        env: mkEnv({}),
        execGhAuthToken: async () => null,
        loadStoredToken: async () => "  ghu_trim  \n",
      });
      expect(r?.token).toBe("ghu_trim");
    });

    it("falls through to gh CLI when stored returns null", async () => {
      const r = await resolveIdentity({
        env: mkEnv({}),
    ...
    it("falls through to gh CLI when stored returns empty string", async () => {
      const r = await resolveIdentity({
        env: mkEnv({}),
        execGhAuthToken: async () => "gho_cli",
  ...
  describe("slot 4 — gh auth token shell-out (fallback)", () => {
    it("uses gh auth token output when no env + no stored", async () => {
      const r = await resolveIdentity({
        env: mkEnv({}),
        execGhAuthToken: async () => "gho_cli",
        loadStoredToken: async () => null,
    ...
    it("returns null when gh also returns null (not installed)", async () => {
      const r = await resolveIdentity({
        env: mkEnv({}),
        execGhAuthToken: async () => null,
        loadStoredToken: async () => null,
  ...
  describe("priority order", () => {
    it("env beats device flow beats gh CLI", async () => {
      const r = await resolveIdentity({
    ...
    it("device flow beats gh CLI", async () => {
      const r = await resolveIdentity({
        env: mkEnv({}),
        execGhAuthToken: async () => "gh_loses",
        loadStoredToken: async () => "device_wins",
