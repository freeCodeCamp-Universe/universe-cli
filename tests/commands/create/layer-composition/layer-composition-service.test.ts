...
const fixtureProvider: TemplateProvider = { loadLayers: loadFromDir };
...
describe(LayerCompositionService, () => {
  ...
  it("emits a Dockerfile for node + express + pnpm", async () => {
    const result = await service.resolveLayers(nodeExpressSelection, FIXTURES_DIR);
  ...
  it("emits a Dockerfile and compose.yaml for static scaffold", async () => {
    const result = await service.resolveLayers(staticSelection, FIXTURES_DIR);
