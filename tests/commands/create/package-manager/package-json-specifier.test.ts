...
import { createPackageSpecifier } from "../../../../src/commands/create/package-manager/package-json-specifier.js";
import type { Runner } from "../../../../src/commands/create/package-manager/package-json-specifier.js";
...
describe(createPackageSpecifier, () => {
  ...
  describe("specifyDeps", () => {
    it("creates a lockfile", async () => {
      ...
      await adapter.specifyDeps(tmpDir, "1.0.0");
    ...
    it("throws UsageError when installLockfileOnly fails", async () => {
      const runner: Runner = {
    ...
    it("throws UsageError when the pinning install fails", async () => {
      let installCount = 0;
      const runner: Runner = {
        async installLockfileOnly(cwd: string) {
          installCount++;
