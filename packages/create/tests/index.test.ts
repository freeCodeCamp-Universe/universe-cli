...
import type { PackageManager, RunOptions } from "../src/package-manager/package-manager.service.js";
import { CreateInputValidationService } from "../src/create-input-validation-service.js";
import type { CreateSelections } from "../src/types.js";
import type { DonationConfigWriter } from "../src/io/donation-config-writer.port.js";
import type { RepoInitialiser } from "../src/io/repo-initialiser.port.js";
import type { SkillInstaller } from "../src/io/skill-installer.port.js";
import type { ResolvedLayerSet } from "../src/layer-composition/layer-composition-service.js";
import { UsageError } from "@freecodecamp/universe-core";
import { loadFromDir } from "../src/layer-composition/template-provider.js";
import { RuntimeSchema } from "../src/layer-composition/schemas/layers.js";
import { resolveTemplateVersions } from "../src/template-version-check.js";
import { ensureTemplateDir } from "../src/layer-composition/template-fetcher.js";
import runtimeFixture from "./fixtures/templates/layers/runtime.json";

vi.mock("@freecodecamp/universe-create", () => import("../src/index.js"));
...
vi.mock("../src/docker-check.js", () => ({
  isDockerAvailable: vi.fn(() => true),
}));

vi.mock("../src/template-version-check.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../src/template-version-check.js")>();
  return { ...actual, resolveTemplateVersions: vi.fn() };
});

vi.mock("../src/layer-composition/template-fetcher.js", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../src/layer-composition/template-fetcher.js")>();
...
describe("create", () => {
  ...
  it("writes the resolved scaffold artifacts to disk", async () => {
    ...
    await createHandler(
      { json: false, yes: true, ...selectionToOptions(createPromptResult) },
      deps,
    );
  ...
  describe("non-interactive mode", () => {
    ...
    it("defaults to the first recommended runtime when --runtime is omitted", async () => {
      ...
      expect({ errors: deps.logError.mock.calls, exits: deps.exit.mock.calls }).toEqual({
        errors: [],
        exits: [],
      });
  ...
  it("succeeds even when Docker daemon is unavailable", async () => {
    const { isDockerAvailable } = await import("../src/docker-check.js");
